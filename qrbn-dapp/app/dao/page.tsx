...
import { ExternalLink, ArrowRight, Users, Building, BookOpen, CheckCircle, XCircle, Loader2, Vote, Coins, FileText } from "lucide-react";
...
export default function DAOPage() {
	...
	const loadUserStats = async () => {
		...
		try {
			...
			setUserStats({
				qrbnBalance: displayTokenPrice(qrbnBalance, 2),
				nftCount: nftBalance.toString(),
				contributionAmount: Number(contributions),
				proposalCount,
			});
							...
							{type !== "sharia" && (
								<>
									...
									<div className="flex items-center gap-2 text-xs">
										...
										<span className="text-[#f0fdf4]/70">Qurban Contributions: {check.contributionAmount} USDT</span>
									</div>
								</>
							)}
							{type === "sharia" && (
								<div className="text-xs text-[#f0fdf4]/70">Special verification process required through application</div>
							)}
						</div>
					)}
					...
					<div>
						...
						<ul className="space-y-1 text-sm">
							...
							{rep.responsibilities.length > 3 && (
								<li className="text-[#f0fdf4]/50 text-xs">+{rep.responsibilities.length - 3} more responsibilities</li>
							)}
						</ul>
					</div>
					...
					<Card className="bg-[#0f2419] border-[#14532d] mb-8">
						...
						<CardContent>
							{loading ? (
								...
								<div className="text-center py-8 text-[#f0fdf4]/70">Failed to load profile data</div>
							)}
						</CardContent>
					</Card>
				)}
